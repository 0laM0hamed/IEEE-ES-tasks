...
struct document get_document(char*text)
{
    ...
    struct word kth_word_in_mth_of_nth(struct document doc,int k,int m,int n)
    {
        return doc.data[n-1].data[m-1].data[k-1];
    }
    struct sentence kth_word_in_mth (struct document doc,int k,int m)
    {
       return doc.data[m-1].data[k-1];
    }
    struct paragraph  kth_word (struct document doc,int k)
    {
        return doc.data[k-1];
    }
...
int main()
{
   ...
   if (type==3)
   {
       int k,m,n;
       scanf("%d %d %d",&k,&m,&n);
       struct word w =kth_word_in_mth_of_nth(doc,k,m,n);
       print_word(w);
   }
else if (type==2)
{
    int k,m;
    scanf("%d %d",&k,&m);
    struct sentence sen =kth_word_in_mth(doc,k,m);
    print_sentence(sen);
}
else
{
    int k;
    scanf("%d",&k);
       struct paragraph para =kth_word(doc,k);
